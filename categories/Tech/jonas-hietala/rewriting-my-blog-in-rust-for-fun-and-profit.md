...
description = "BackgroundI’ve used Hakyll as my static site generator for around 9 years now. Before that I think I used Jekyll and also more dynamic pages with Mojolicious in Perl and "
date = "2025-01-18T19:20:23Z"
url = "http://jonashietala.se/blog/2022/08/29/rewriting_my_blog_in_rust_for_fun_and_profit/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.357871649Z"
seen = true
---
...
I’ve used [Hakyll](https://jaspervdj.be/hakyll/) as my static site generator for around 9 years now. Before that I think I used [Jekyll](https://jekyllrb.com/) and also more dynamic pages with [Mojolicious](https://mojolicious.org/) in Perl and [Kohana](https://kohanaframework.org/) in PHP, but I can’t be completely sure as the git history doesn’t go back that far.

But all good things come to an end and I’ve now migrated to my own custom site generator written in Rust.

### [Problems with my previous setup](#Problems-with-my-previous-setup) ###
...
2. Several external dependencies.
   ...
   While the site generator itself is written in Haskell, there were other dependencies apart from a number of Haskell libs. My blog helper script was written in Perl, I used [sassc](https://github.com/sass/sassc) to convert sass, [pygments](<https://pygments.org/ "Python syntax highlighter">) in Python for syntax highlighting, and [s3cmd](<https://github.com/s3tools/s3cmd "S3cmd tool for Amazon Simple Storage Service (S3)">) for uploading the generated site to S3.
...
3. Setup problems.
   ...
   * After GHC is updated it [can no longer find cabal packages](/blog/2020/05/09/ghc_cannot_find_cabal_packages/).
   ...
   * When Hakyll changed Pandoc arguments between versions, [breaking code rendering in my Atom feed](https://github.com/jaspervdj/hakyll/issues/662).
...
4. Mental overhead with Haskell.
   ...
   Even if you don’t understand the `$` and `>>=`, I still think it’s clear that we’re finding files from the `static/` folder, sending them to `pandocCompiler` (to convert from markdown), to some templates and then de-indexing urls (to avoid links ending with `index.html`).
...
### [Why reinvent the wheel again?](#Why-reinvent-the-wheel-again) ###
...
(Yes I know that projects like [cobalt](https://cobalt-org.github.io/docs/) allows you to make any kind of transformation, using whatever language you want, to your pages.)

[Implementation details](#Implementation-details)
----------

I’m not going to describe everything I did here, take a look at the [source code](https://github.com/treeman/jonashietala) if you’re interested.

### [Delegate the hard things](#Delegate-the-hard-things) ###
...
* [tera](https://tera.netlify.app/) for a templating engine.
...
* [pulldown-cmark](https://crates.io/crates/pulldown-cmark) for parsing Markdown.
  ...
  It’s really for [CommonMark](https://commonmark.org/) which is a standard, unambiguous syntax specification for Markdown.

  While fast, it doesn’t support as many things as [Pandoc](https://pandoc.org/MANUAL.html) does, so I had to extend it with some more features. More on that later.

* [syntect](https://crates.io/crates/syntect) for syntax highlighting, supporting Sublime Text’s grammars.

* [yaml-front-matter](https://crates.io/crates/yaml-front-matter) to parse metadata from posts.

* [grass](https://crates.io/crates/grass) as a Sass compiler purely in Rust.

* [axum](https://crates.io/crates/axum) to create [a static file server](https://github.com/tokio-rs/axum/tree/main/examples/static-file-server) used to host the site locally.

* [hotwatch](https://crates.io/crates/hotwatch) to watch for file changes, so we can update pages whenever a file is updated.

* [scraper](https://crates.io/crates/scraper) to parse the generated html. Used in some of my tests and for some specific transformations.

* [rust-s3](https://crates.io/crates/rust-s3) to upload the generated site to S3 storage.
...
### [Markdown transformations](#Markdown-transformations) ###

While it would be easy if my posts were just standard markdown, over the years I’ve included various features and extensions that my markdown parsers of choice [pulldown-cmark](https://crates.io/crates/pulldown-cmark) don’t support. So I had to code them myself.
...
#### [Extending pulldown\_cmark](#Extending-pulldown_cmark) ####

I’ve also extended [pulldown-cmark](https://crates.io/crates/pulldown-cmark) with transformations of my own:
...
Demoting headers and [embedding bare YouTube links](/blog/2014/09/01/embedding_youtube_videos_with_hakyll/) is something I used to do before, and were fairly straightforward do implement. (It might have been better to embed YouTube links in a pre- or post-processing step though.)
...
That would call a `notice` parser, which in this case would create a `<aside>` tag instead of a `<blockquote>` tag, while preserving the parsed markdown.

While there are existing crates that adds code highlighting using [syntect], I wrote my own that wraps it in a `<code>` tag and supports inline code highlighting. For example “Inside row: `let x = 2;`” is produced by:
...
The first thing is that if you use [syntect] and have custom syntaxes, you really should [compress `SyntaxSet` to a binary format](https://docs.rs/syntect/latest/syntect/dumps/index.html).

The other thing was to parallelize rendering using [rayon](https://docs.rs/rayon/latest/rayon/). Rendering is the markdown parsing, applying templates and creating the output file. Rayon is great is this task is limited by CPU and it’s very easy to use (if the code is structured correctly). This is for instance a simplified view of the rendering:
...
4. ****I had an easier time with Rust****
   ...
   While I did find it easier to implement things like [series](/series) or prev/next links (which aren’t live yet as I haven’t styled them properly), I don’t think it means that Rust is simpler or easier than Haskell, it just means that I personally had an easier time to grok Rust than Haskell.
