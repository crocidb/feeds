...
description = "One of my favorite features in Neovim is the Tree-sitter integration. It allows for fast syntax highlighting that works well even in an error state (often the case when you’re editing code), and it has additional semantics (you can differentiate between function parameters and lo"
date = "2025-05-01T11:33:54Z"
url = "http://jonashietala.se/blog/2024/03/19/lets_create_a_tree-sitter_grammar/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.239969083Z"
seen = true
---
...
With [nvim-treesitter-textobjects](https://github.com/nvim-treesitter/nvim-treesitter-textobjects) you can also jump between nodes (such as `]c` to jump to next class) or target deletion (`cif` to delete the function body and enter insert mode). An amazing feature as it works across languages, no matter how they look like, as long as they have a Tree-sitter grammar (most do).

But, you might wonder what does a Tree-sitter grammar look like, and how do you create one for a language?

I started thinking about this and before I knew it I was [trying to make my own parser](https://github.com/treeman/tree-sitter-djot) for [Djot](https://djot.net/) (a markup language similar to Markdown). There are some good tutorials on how to get started, but not on some more advanced things.
...
2. Using Tree-sitter’s built-in conflicts resolution
3. Syntax highlighting with language injection
4. Use the grammar from Neovim for syntax highlighting and textobjects
5. Embed the grammar into this Blog for syntax highlighting

The source code for the complete grammar we’ll develop [can be found on GitHub](https://github.com/treeman/tree-sitter-sdjot).

[Our subset](#Our-subset)
----------

For the purpose of this blog post, we’ll implement a small subset of [Djot](https://djot.net/), where we’ll support:
...
2. Closing a div should close other open blocks (divs and paragraphs in our case).
...
2. Setup `externals` tokens in `grammar.js` and a matching C enum in `scanner.c`.
3. Define and implement five C functions.
...
C is scary. Try [valgrind](https://valgrind.org/).
