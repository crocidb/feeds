...
description = "This is the revision of the T-34 keyboard layout that I use as my primary driver. I’ve used this particular version without any major changes for almost 3 months, so I’d say it’s a good improvement over the last revision.There are two large changes from [T-34/"
date = "2024-11-25T19:34:02Z"
url = "http://jonashietala.se/blog/2021/12/15/t-34-1/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.368067942Z"
seen = true
---

This is the revision of the [T-34](/blog/tags/t-34/) keyboard layout that I use as my primary driver. I’ve used this particular version without any major changes for almost 3 months, so I’d say it’s a good improvement over the last revision.

There are two large changes from [T-34/0](/blog/2021/09/05/t-34-0/): swapping `o` and `Repeat` and preferring modifiers on combos over [callum-style mods](https://github.com/callum-oakley/qmk_firmware/tree/master/users/callum#oneshot-modifiers).
...
![](/images/t-34-1/base.png) Base layer
...
2. `Repeat` is still [on the top row](/blog/2021/09/05/t-34-0#numbers) on the numbers layer, which breaks symmetry and hurts learning.
...
4. Moving `o` away from the home row is unfortunate as it is one of the [most common keys](/blog/2021/09/05/t-34-0#the-repeat-key). It’s not the end of the world though as `Repeat` is also [quite common](/blog/2021/09/05/t-34-0#the-repeat-key), the `ao` bigram isn’t that problematic, and rolling `io` is still okay. It also makes the `ou` roll great.
...
Another thing I’ve been slightly annoyed with is mods. I’ve been using [callum-style mods](https://github.com/callum-oakley/qmk_firmware/tree/master/users/callum#oneshot-modifiers), where you essentially press the inner thumb key and then a mod on the home-row. There are no timings here, so you can tap them as quickly as you can.
...
I did use a keylog when I started creating the layout, but it didn’t tell me much modifier wise; `Shift`, `Ctrl` and `Gui` all had about the same amount of usage, and for some reason `Alt` was never registered. Instead, I used my [dotfiles](https://github.com/treeman/dotfiles) and used ~~the force~~ felt out a modifier ranking:
...
2. `Gui`. I use `Gui` as my window manager prefix (to change windows, workspaces, open a terminal etc). A dedicated mod key isn’t as important as I have [dedicated navigation layers](/blog/2021/06/03/the-t-34-keyboard-layout#Navigation), but still.
...
4. While `Shift` was (slightly) the most used modifier in my keylog, I’ve moved on to [auto shift](https://docs.qmk.fm/#/feature_auto_shift) so it’s not at all important. There’s still some usage for it, for instance to `Shift` + mouse select in file browsers, so it’s good to have especially on the left side.
...
<img height="180" src="/images/t-34-1/lgui.png"> <img height="180" src="/images/t-34-1/rgui.png"> <img height="180" src="/images/t-34-1/lctrl.png"> <img height="180" src="/images/t-34-1/rctrl.png"> <img height="180" src="/images/t-34-1/lshft.png"> <img height="180" src="/images/t-34-1/rshft.png"> <img height="180" src="/images/t-34-1/lctrl-shift.png"> <img height="180" src="/images/t-34-1/rctrl-shift.png">
...
![](/images/t-34-1/rmod.png) Mods & symbols
...
![](/images/t-34-1/lmod.png) Shortcuts & symbols
...
* Instant leader keys for options.
  ...
  A [leader key](https://docs.qmk.fm/#/feature_leader_key), is a nice idea as you can use several keypresses to trigger a single function. For instance you can have:
  ...
  I have tried [QMK’s leader key](https://docs.qmk.fm/#/feature_leader_key) before, but with little success. It’s based on timeouts, which I for some reason found really difficult to consistently trigger (and they took a lot of memory too).

  But there is a [userspace leader sequence](https://github.com/andrewjrae/kyria-keymap#userspace-leader-sequences) that I’d like to try instead, which seems like it would solve the timing issues I had.
