...
description = "An early, small hurdle diving into WebAssembly was allocating my allocator. On a server or desktop with virtual memory, the allocator asks the operating system to map fresh pages into its address space ([sbrk](https://pubs.opengroup.org/onlinepubs/7908799/xsh"
date = "2025-04-19T03:18:20Z"
url = "https://nullprogram.com/blog/2025/04/19/"
author = "Christopher Wellons"
text = ""
lastupdated = "2025-10-22T08:59:04.646608273Z"
seen = true
---
...
For the purposes of this discussion, the actual allocator isn’t important. It could be [a simple arena allocator](/blog/2023/09/27/), or a more general purpose [buddy allocator](https://github.com/skeeto/scratch/blob/master/misc/buddy.c). It could even be garbage collected with [Boehm GC](https://www.hboehm.info/gc/). Though WebAssembly’s linear memory is a poor fit for such a [conservative](https://en.wikipedia.org/wiki/Tracing_garbage_collection#Precise_vs._conservative_and_internal_pointers) garbage collector. In a compact address space starting at zero, and which doesn’t include code, memory addresses will be small numbers, and less distinguishable from common integer values. There’s also the issue that the garbage collector cannot scan the Wasm stack, which is hidden from Wasm programs by design. Only the ABI stack is visible. So a garbage collector requires cooperation from the compiler — essentially as a distinct calling convention — to spill all heap pointers on the ABI stack before function calls. Wasm C and C++ toolchains do not yet support this in a practical capacity.
...
Wasm programs can grow linear memory using an [sbrk](https://pubs.opengroup.org/onlinepubs/7908799/xsh/brk.html)-like [`memory.grow` instruction](https://webassembly.github.io/spec/core/bikeshed/#syntax-instr-memory). It operates in quantities of pages (64kB), and returns the old memory size. Because memory starts at zero, the old memory size is also the base address of the new allocation. Clang provides access to this instruction via an undocumented built-in:
...
The first parameter selects a memory because [someday there might be more than one](https://github.com/WebAssembly/multi-memory/blob/main/proposals/multi-memory/Overview.md). From this built-in we can define `sbrk`:
