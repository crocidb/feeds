...
description = "To recap, in the first part of this two-part tutorial, we built a simple JSON REST web service in PureScript to create, update, get, list and delete user"
date = "2017-10-01T00:00:00Z"
url = "https://abhinavsarkar.net/posts/ps-simple-rest-service-2/"
...
This post was originally published on [abhinavsarkar.net](https://abhinavsarkar.net/posts/ps-simple-rest-service-2/?mtm_campaign=feed).

This post is a part of the series: **Writing a Simple REST Web Service in PureScript**.

1. [Part 1](https://abhinavsarkar.net/posts/ps-simple-rest-service/?mtm_campaign=feed)
2. **Part 2** 👈
...
We get back a default HTML response with a 404 status from [Express](https://expressjs.com). Since we are writing a JSON API, we should return a JSON response in this case too. We add the following code in the `src/SimpleService/Server.purs` file to add a catch-all route and send a 404 status with a JSON error message:
...
`allRoutePattern` matches all routes because it uses a `"/.*"` [regular expression](https://en.wikipedia.org/wiki/Regular_expression). We place it as the last route to match all the otherwise unrouted requests. Let’s see what is the result:
...
We add the `useOnError` handler which comes with [`purescript-express`](https://pursuit.purescript.org/packages/purescript-express) to return the error message as a JSON response. Back on the command-line:
...
The validation logic has been extracted out in separate functions now which are composed using [Applicative](https://pursuit.purescript.org/packages/purescript-prelude/3.0.0/docs/Control.Applicative#t:Applicative). The validation steps are composed using the `ExceptT` monad. We are now free to express the core logic of the function clearly. We rewrite the `src/SimpleService/Handler.purs` file using the validations:
...
We are going to extract it out of the code and read it from the environment variables using the [`purescript-config`](https://pursuit.purescript.org/packages/purescript-config) package. First, we install the required packages using [bower](http://bower.io).
...
For logging, we use the [`purescript-logging`](https://pursuit.purescript.org/packages/purescript-logging) package. We write a logger which logs to `stdout`; in the `src/SimpleService/Logger.purs` file:
...
In this tutorial we learned how to create a simple JSON REST web service written in PureScript with persistence, validation, configuration and logging. The complete code for this tutorial can be found in [github](https://github.com/abhin4v/ps-simple-rest-service).
...
* [My website](https://abhinavsarkar.net/posts/ps-simple-rest-service-2/?mtm_campaign=feed#comment-container)
