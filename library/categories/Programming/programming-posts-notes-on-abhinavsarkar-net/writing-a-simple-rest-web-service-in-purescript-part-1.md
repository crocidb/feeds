...
description = "At Nilenso, we’ve been working with a client who has chosen PureScript as their primary programming language. Since I couldn’t find any canonical documentation on writing a web service in PureScript, I thought I’d jot down the "
date = "2017-09-29T00:00:00Z"
url = "https://abhinavsarkar.net/posts/ps-simple-rest-service/"
...
The aim of this two-part tutorial is to create a simple JSON [REST](https://en.wikipedia.org/wiki/REST) web service written in PureScript, to run on a node.js server. This assumes that you have basic proficiency with PureScript.

This post was originally published on [abhinavsarkar.net](https://abhinavsarkar.net/posts/ps-simple-rest-service/?mtm_campaign=feed).

This post is a part of the series: **Writing a Simple REST Web Service in PureScript**.

1. **Part 1** 👈
2. [Part 2](https://abhinavsarkar.net/posts/ps-simple-rest-service-2/?mtm_campaign=feed)
...
In this part we’ll work on setting up the project and on the first two requirements. In the [next](https://abhinavsarkar.net/posts/ps-simple-rest-service-2/?mtm_campaign=feed) part we’ll work on the rest of the requirements.

### Contents ###

1. [Setting Up](#setting-up)
2. [Types First](#types-first)
3. [Persisting It](#persisting-it)
4. [Serving It](#serving-it)
   1. [Getting a User](#getting-a-user)
   2. [Deleting a User](#deleting-a-user)
   3. [Creating a User](#creating-a-user)
   4. [Updating a User](#updating-a-user)
   5. [Listing all Users](#listing-all-users)
...
We start with installing PureScript and the required tools. This assumes that we have [node](https://nodejs.org) and [npm](https://www.npmjs.com) installed on our machine.
...
[Pulp](https://github.com/purescript-contrib/pulp) is a build tool for PureScript projects and [bower](http://bower.io) is a package manager used to get PureScript libraries. We’ll have to add `~/.local/bin` in our `$PATH` (if it is not already added) to access the binaries installed.
...
We are using the generic support for PureScript types from the [`purescript-generics-rep`](https://pursuit.purescript.org/packages/purescript-generics-rep) and [`purescript-foreign-generic`](https://pursuit.purescript.org/packages/purescript-foreign-generic) libraries to encode and decode the `User` type to JSON. We install the library by running the following command:
...
We use `encodeJSON` and `decodeJSON` functions from the [`Data.Foreign.Generic`](https://pursuit.purescript.org/packages/purescript-foreign-generic/4.3.0/docs/Data.Foreign.Generic) module to encode and decode the `User` instance to JSON. The return type of `decodeJSON` is a bit complicated as it needs to return the parsing errors too. In this case, the decoding returns no errors and we get back a `Right` with the correctly parsed `User` instance.

Persisting It[#](#persisting-it)
----------

Next, we add the support for saving a `User` instance to a Postgres database. First, we install the required libraries using bower and npm: [`pg`](https://github.com/brianc/node-postgres) for Javascript bindings to call Postgres, [`purescript-aff`](https://pursuit.purescript.org/packages/purescript-aff) for asynchronous processing and [`purescript-postgresql-client`](https://pursuit.purescript.org/packages/purescript-postgresql-client) for PureScript wrapper over `pg`:
...
We create the `databaseConfig` record with the configs needed to connect to the database. Using the record, we create a new Postgres connection pool (`PG.newPool`) and get a connection from it (`PG.withConnection`). We call `PG.execute` with the connection, the SQL insert query for the users table and the `User` instance, to insert the user into the table. All of this is done inside [`launchAff`](https://pursuit.purescript.org/packages/purescript-aff/3.1.0/docs/Control.Monad.Aff#v:launchAff) which takes care of sequencing the callbacks correctly to make the asynchronous code look synchronous.
...
We can now write a simple HTTP API over the persistence layer using [Express](https://expressjs.com) to provide CRUD functionality for users. Let’s install Express and [purescript-express](https://pursuit.purescript.org/packages/purescript-express), the PureScript wrapper over it:
...
We can test this on the command-line using [HTTPie](https://httpie.io). We run `pulp --watch run` in one terminal to start the server with file watching, and test it from another terminal:
...
`createUser` handler is a bit more involved. First, we add an Express middleware to parse the body of the request as JSON. We use [`body-parser`](https://github.com/expressjs/body-parser) for this and access it through PureScript [FFI](https://github.com/purescript/documentation/blob/master/guides/FFI.md). We create a new file `src/SimpleService/Middleware/BodyParser.js` with the content:
...
`createUser` calls [`getBody`](https://pursuit.purescript.org/packages/purescript-express/0.5.2/docs/Node.Express.Request#v:getBody) which has type signature `forall e a. (Decode a) => HandlerM (express :: EXPRESS | e) (Either MultipleErrors a)`. It returns either a list of parsing errors or a parsed instance, which in our case is a `User`. In case of errors, we just return the errors rendered as string with a 422 status. If we get a parsed `User` instance, we do some validations on it, returning appropriate error messages. If all validations pass, we create the user in the database by calling `insertUser` from the persistence layer and respond with a status 201.
...
[`NullOrUndefined`](https://pursuit.purescript.org/packages/purescript-foreign-generic/4.3.0/docs/Data.Foreign.NullOrUndefined#t:NullOrUndefined) is a wrapper over `Maybe` with added support for Javascript `null` and `undefined` values. We define `UserPatch` as having a possibly null (or undefined) `name` field.
...
That concludes the first part of the two-part tutorial. We learned how to set up a PureScript project, how to access a Postgres database and how to create a JSON REST API over the database. The code till the end of this part can be found in [github](https://github.com/abhin4v/ps-simple-rest-service/tree/9fdfe3a15508a3c29bd4bc96310fcf52b1022678). In the [next](https://abhinavsarkar.net/posts/ps-simple-rest-service-2/?mtm_campaign=feed) part, we’ll learn how to do API validation, application configuration and logging.
...
* [Reddit](https://www.reddit.com/r/purescript/comments/737bg1/)
* [My website](https://abhinavsarkar.net/posts/ps-simple-rest-service/?mtm_campaign=feed#comment-container)
