...
description = "Linked lists are a data structure basic building block, with especially flexible allocation behavior. They’re not just a useful starting point, but sometimes a sound foundation for future growth. I’m going to start with the beginner stuff, then *without disrupting the original li"
...
For the sake of an interesting example, I’m will demonstrate with the same concept as [last time I talked about data structures](/blog/2025/01/19/): a collection of key/value strings, in the form of an environment variables. This time in linked list form:
...
And all the parser heavy lifting will be done by [our ever-handy `cut` function](/blog/2025/03/02/):
...
If you’re doing many look-ups, or if the list is long, those linear scans to find items in the list are not ideal. We can introduce an intrusive hash map, in the form of [a hash trie](/blog/2023/09/30/), by adding two more pointers to the linked list:
...
**Update**: Thanks to [Daniel Kareh for a correction](https://lists.sr.ht/~skeeto/public-inbox/<SJ2PR12MB79208563F4485DCAA27D5776A2BAA@SJ2PR12MB7920.namprd12.prod.outlook.com>?__goaway_challenge=meta-refresh&__goaway_id=5902363e020028d0488062799debf13b&__goaway_referer=https://lists.sr.ht/~skeeto/public-inbox).
...
If the list is static once constructed, or if look-ups happen much more frequently than the list grows, we can find list items even faster by constructing an index table over the list: [an MSI hash table](/blog/2022/08/08/). This table avoids redundancy by *sharing structure with the list*. Because it’s a flat table, if we keep adding to the list then eventually we’ll need to reconstruct a larger table when it becomes overloaded.
...
With these techniques at hand, I can start with linked lists when they are convenient, and later add needed features without fundamentally changing the underlying data structure. None of this requires runtime support, and so it fits comfortably on embedded systems, tiny WebAssembly programs, etc. All the above code is available ready to run: [`list.c`](https://gist.github.com/skeeto/493823d5956dfdc1d95d8c390c2b0e1d).