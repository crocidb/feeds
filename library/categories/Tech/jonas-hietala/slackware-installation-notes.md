...
description = "It seems like it’s been a yearly recurrence reinstalling slackware from scratch. This time it happened during my vacation when I was mucking around with trying to compile erlang with wxWidgets suppo"
date = "2024-07-17T19:35:07Z"
url = "http://jonashietala.se/blog/2016/07/29/slackware_installation_notes/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.509824626Z"
seen = true
---

It seems like it’s been a [yearly](/blog/2015/08/02/slackware_update/) [recurrence](/blog/2014/07/05/reinstalling_slackware/) reinstalling slackware from scratch. This time it happened during my vacation when I was mucking around with trying to compile erlang with wxWidgets support and somehow a make clean started to remove / and I only noticed it too late… Not sure how it happened but I had changed things in the make and config files. Oh well no data lost just annoyances.
...
Even with only a laptop with windows installed hope is not lost. The almighty alien has a [great tutorial](http://alien.slackbook.org/blog/welcome-windows-user/) creating an usb boot loader from windows. At first I tried to extract the contents of usbboot.img using [IZarc](https://www.izarc.org/) but it reported it as empty but [Winimage](http://www.winimage.com/winimage.htm) worked fine. The rest went smoothly.
...
The vast majority of packages I install can be found on [Slackbuilds](https://slackbuilds.org/). Many unnamed dependencies can simply be found here. [sbopkg](http://www.sbopkg.org/) makes it a lot easier to use.
...
Store [https://github.com/treeman/dotfiles](https://github.com/treeman/dotfiles) in `~/dotfiles` and symlink from there as needed.
...
Clone [https://github.com/robm/dzen](https://github.com/robm/dzen) and edit `config.mk`, choose option 7 (XPM, XFT, Xinerama). I’m not 100% I need to do it for the laptop or if this only was needed to support Xinerama but I did it this way anyway.

When started use `lxappearance` to set a prettier look for firefox and other gui. See [previous post](/blog/2014/07/05/reinstalling_slackware/#Fonts) about prettifying fonts.
