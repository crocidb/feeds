...
description = " The T-34/2 layout.   Generated with the QMK Heatmap Generator, with all layers and combos.It’s been 6 months since my last update on the T-34 keyboa"
date = "2024-11-25T19:34:02Z"
url = "http://jonashietala.se/blog/2022/08/28/the_t-342_keyboard_layout/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.361407559Z"
seen = true
---
...
The T-34/2 layout.  
 Generated with the [QMK Heatmap Generator](https://precondition.github.io/qmk-heatmap#how-to-collect-the-required-data), with all layers and combos.

It’s been 6 months since my last update on the [T-34](/blog/tags/t-34/) keyboard layout, and I’ve made some changes I’d like to document here.
...
![](/images/t-34-2/legend.png) Legend

Swap AO and IU (again)
==========

![](/images/t-34-2/base.png) Base

In my [last iteration](/blog/2021/12/15/t-34-1/) I had `iu` on my middle finger and `ao` on my ring finger. That was far from optimal as the ring finger is stronger, and `ao` is much more common for me. It was just an oversight after I swapped `Repeat` to the home-row (from where `o` is now) but it has now been corrected.
...
While [T-34/1](/blog/2021/12/15/t-34-1/) had symbols and mods on the thumbs (only used for multiples, as I have single combos for them all) I now only have rarely used keys under the layers there:

![](/images/t-34-2/shrt.png) Shortcuts under the left thumb.
...
![](/images/t-34-2/spec.png) Specials under the right thumb (with ←↓↑→ symbols, not arrow keys).
...
![](/images/t-34-2/hcombos.png) Horizontal combos with nearby keys.
...
![](/images/t-34-2/mods.png) Modifiers

Symbols
----------

Another consequence of the change is that the two symbol layers (that [used to](/blog/2021/06/03/the-t-34-keyboard-layout/#Mods-symbols) have [callum style mods](https://github.com/callum-oakley/qmk_firmware/tree/master/users/callum#oneshot-modifiers) on the opposite side) are now combined into a single layer:

![](/images/t-34-2/sym.png) Symbols
...
A [leader key](https://docs.qmk.fm/#/feature_leader_key) is a function that triggers after a sequences of keys. So for instance I press `Leader`, then `t` and finally `n` to toggle the number layer. I use these sequences with the combo `l` + `)` as my leader key:
...
I don’t use QMK’s version, as I couldn’t get used to the timeouts, but a [userspace implementation](https://github.com/andrewjrae/kyria-keymap#userspace-leader-sequences) that resolves instantly.
...
The [QMK Heatmap Generator](https://precondition.github.io/qmk-heatmap#how-to-collect-the-required-data) also provides heatmaps for the individual layers, which gives another way to analyze the keymap compared to an indiscriminate keylogger I’ve used before.

Note that I did not take these heatmaps into account making this version of the keymap.

![](/images/t-34-2/freq/base.png) Base layer (this includes virtually all combos too).

Can you feel the `j`/`k` abuse?

![](/images/t-34-2/freq/swe.png) Swedish layer (only ÅÄÖ are changed from base).
...
![](/images/t-34-2/freq/sym.png) Symbols layer.
...
![](/images/t-34-2/freq/num.png) Numbers layer.

Note that this is only used together with [NUMWORD](/blog/2021/06/03/the-t-34-keyboard-layout/#Where-are-the-digits), and single digits are typed with thumb combos—such as `0` that in Vim moves the cursor to the beginning of the line.
...
![](/images/t-34-2/rel_vim.png) Activate [NUMWORD](/blog/2021/06/03/the-t-34-keyboard-layout/#Where-are-the-digits), then `4j` would move the cursor 4 lines down to `.collect` and turn off the number layer. ![](/images/t-34-2/freq/nav.png) Navigation layer.
...
![](/images/t-34-2/freq/wnav.png) Workspace nav layer.
...
As usual, the [firmware QMK code](https://github.com/treeman/qmk_firmware/tree/master/keyboards/ferris/keymaps/treeman) is on GitHub.