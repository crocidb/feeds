...
description = "Sudoku is a number placement puzzle. It consists of a 9x9 grid which is to be filled with digits from 1 to 9. Some of the cells of the grid come pre-filled and the player has to fill the rest.Haskell is a purely "
...
[Haskell](https://www.haskell.org/) is a purely functional programming language. It is a good choice to solve Sudoku given the problem’s [combinatorial](https://en.wikipedia.org/wiki/Combinatorics) nature. The aim of this series of posts is to write a **fast** Sudoku solver in Haskell. We’ll focus on both implementing the solution and making it efficient, step-by-step, starting with a slow but simple solution in this post[<sup>1</sup>](#fn1).

This post was originally published on [abhinavsarkar.net](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-1/?mtm_campaign=feed).
...
1. [Constraint Satisfaction Problem](#constraint-satisfaction-problem)
2. [Setting up](#setting-up)
3. [Pruning the Cells](#pruning-the-cells)
4. [Pruning the Grid](#pruning-the-grid)
5. [Making the Choice](#making-the-choice)
6. [Solving the Puzzle](#solving-the-puzzle)
7. [Conclusion](#conclusion)

Constraint Satisfaction Problem[#](#constraint-satisfaction-problem)
----------

Solving Sudoku is a [constraint satisfaction problem](https://en.wikipedia.org/wiki/Constraint_satisfaction_problem). We are given a partially filled grid which we have to fill completely such that each of the following constraints are satisfied:
...
1. If a cell contains a digit, remove that digit from the list of the possible digits from all its neighboring cells. Neighboring cells are the other cells in the given cell’s row, column and sub-grid. For example, the grid after removing the fixed value `4` of the row-2-column-1 cell from its neighboring cells:

![](data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20921%20209'%3E%3C/svg%3E) \<img src="/images/fast-sudoku-solver-in-haskell-1/sudoku2.svg" class="w-100pct nolink extra-width"\>\</img\>

1. Repeat the previous step for all the cells that are have been solved (or *fixed*), either pre-filled or filled in the previous iteration of the solution. For example, the grid after removing all fixed values from all non-fixed cells:

![](data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20921%20209'%3E%3C/svg%3E) \<img src="/images/fast-sudoku-solver-in-haskell-1/sudoku3.svg" class="w-100pct nolink extra-width"\>\</img\>

1. Continue till the grid *settles*, that is, there are no more changes in the possibilities of any cells. For example, the settled grid for the current iteration:
...
1. Once the grid settles, choose one of the non-fixed cells following some strategy. Select one of the digits from all the possibilities of the cell, and fix (assume) the cell to have that digit. Go back to step 1 and repeat.
2. The elimination of possibilities may result in inconsistencies. For example, you may end up with a cell with no possibilities. In such a case, discard that branch of solution, and backtrack to last point where you fixed a cell. Choose a different possibility to fix and repeat.
3. If at any point the grid is completely filled, you’ve found the solution!
4. If you exhaust all branches of the solution then the puzzle is unsolvable. This can happen if it starts with cells pre-filled wrongly.

This algorithm is actually a [Depth-First Search](https://en.wikipedia.org/wiki/Depth-first_search) on the [state space](https://en.wikipedia.org/wiki/State_space_search) of the grid configurations. It guarantees to either find a solution or prove a puzzle to be unsolvable.
...
A cell is either fixed with a particular digit or has a set of digits as possibilities. So it is natural to represent it as a [sum type](https://en.wikipedia.org/wiki/Algebraic_data_type) with `Fixed` and `Possible` constructors. A row is a list of cells and a grid is a list of rows.
...
Pruning the Cells[#](#pruning-the-cells)
----------
...
We use the [`traverse`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Traversable.html#v:traverse) function for pruning the cells so that a `Nothing` resulting from pruning one cell propagates to the entire list.
...
> :}
> putStr $ showGridWithPossibilities $ [head grid] -- first row of the grid
6           [123456789] [123456789] [123456789] [123456789] [123456789] [123456789] 1           [123456789]
> putStr $ showGridWithPossibilities [fromJust $ pruneCells $ head grid] -- same row after pruning
...
Pruning the Grid[#](#pruning-the-grid)
----------
...
>   readGrid "6......1.4.........2...........5.4.7..8...3....1.9....3..4..2...5.1........8.6..."
> :}
> Just grid' = traverse pruneCells grid
> putStr $ showGridWithPossibilities grid'
6           [ 2345 789] [ 2345 789] [ 2345 789] [ 2345 789] [ 2345 789] [ 2345 789] 1           [ 2345 789]
...
By `traverse`-ing the grid with `pruneCells`, we are able to prune each row, one-by-one. Since pruning a row doesn’t affect another row, we don’t have to pass the resulting rows between each pruning step. That is to say, `traverse` is enough for us, we don’t need [`foldl`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Foldable.html#v:foldl) here.

How do we do the same thing for columns now? Since our representation for the grid is rows-first, we first need to convert it to a columns-first representation. Luckily, that’s what [`Data.List.transpose`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-List.html#v:transpose) function does:

```
> Just grid = readGrid "693784512487512936125963874932651487568247391741398625319475268856129743274836159"
...
8 5 6 1 2 9 7 4 3
2 7 4 8 3 6 1 5 9
> putStr $ showGrid $ Data.List.transpose grid
6 4 1 9 5 7 3 8 2
9 8 2 3 6 4 1 5 7
3 7 5 2 8 1 9 6 4
7 5 9 6 2 3 4 1 8
...
>   readGrid "6......1.4.........2...........5.4.7..8...3....1.9....3..4..2...5.1........8.6..."
> :}
> Just grid' = fmap Data.List.transpose . traverse pruneCells . Data.List.transpose $ grid
> putStr $ showGridWithPossibilities grid'
...
First, we `transpose` the grid to convert the columns into rows. Then, we prune the rows by `traverse`-ing `pruneCells` over them. And finally, we turn the rows back into columns by `transpose`-ing the grid back again. The last `transpose` needs to be [`fmap`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Prelude.html#v:fmap)-ped because `traverse pruneCells` returns a `Maybe`.
...
8 5 6 1 2 9 7 4 3
2 7 4 8 3 6 1 5 9
> putStr $ showGrid $ subGridsToRows grid
6 9 3 4 8 7 1 2 5
7 8 4 5 1 2 9 6 3
5 1 2 9 3 6 8 7 4
9 3 2 5 6 8 7 4 1
...
8 5 6 1 2 9 7 4 3
2 7 4 8 3 6 1 5 9
> putStr $ showGrid $ subGridsToRows $ subGridsToRows $ grid
6 9 3 7 8 4 5 1 2
4 8 7 5 1 2 9 3 6
1 2 5 9 6 3 8 7 4
9 3 2 6 5 1 4 8 7
...
>   readGrid "6......1.4.........2...........5.4.7..8...3....1.9....3..4..2...5.1........8.6..."
> :}
> Just grid' = fmap subGridsToRows . traverse pruneCells . subGridsToRows $ grid
> putStr $ showGridWithPossibilities grid'
...
It works well. Now we can string together these three steps to prune the entire grid. We also have to make sure that result of pruning each step is fed into the next step. This is so that the fixed cells created into one step cause more pruning in the further steps. We use monadic bind ([`>>=`](https://hackage.haskell.org/package/base-4.10.1.0/docs/Control-Monad.html#v:-62--62--61-)) for that. Here’s the final code:
...
We can clearly see the massive pruning of possibilities all around the grid. We also see a `7` pop up in the row-7-column-5 cell. This means that we can prune the grid further, until it settles. If you are familiar with Haskell, you may recognize this as trying to find a [fixed point](https://en.wikipedia.org/wiki/Fixed_point_(mathematics)) for the `pruneGrid'` function, except in a monadic context. It is simple to implement:
...
1. the cell has only two possible values, resulting in two fixed cells, or,
2. the cell has more than two possible values, resulting in one fixed and one non-fixed cell.
...
[ 2    789] 5           [ 2 4 67 9] 1           [ 23      ] [ 23     9] [     6789] [  34 6789] [  34 6 89]
[12    7 9] [1  4  7 9] [ 2 4  7 9] 8           [ 23      ] 6           [1   5 7 9] [  345 7 9] [1 345   9]
> -- the row-4-column-1 cell is the first cell with only two possibilities, [2, 9].
> -- it is chosen as the pivot cell to find the next grids.
> (grid1, grid2) = nextGrids grid'
> putStr $ showGridWithPossibilities grid1
6           [  3   789] [  3 5 7 9] [ 23 5 7 9] [ 234   8 ] [ 2345 789] [    5 789] 1           [ 2345  89]
4           [1 3   789] [  3 5 7 9] [ 23 567 9] [123  6 8 ] [123 5 789] [    56789] [ 23 56789] [ 23 56 89]
[1   5 789] 2           [  3 5 7 9] [  3 567 9] [1 34 6 8 ] [1 345 789] [    56789] [  3456789] [  3456 89]
...
[ 2    789] 5           [ 2 4 67 9] 1           [ 23      ] [ 23     9] [     6789] [  34 6789] [  34 6 89]
[12    7 9] [1  4  7 9] [ 2 4  7 9] 8           [ 23      ] 6           [1   5 7 9] [  345 7 9] [1 345   9]
> putStr $ showGridWithPossibilities grid2
6           [  3   789] [  3 5 7 9] [ 23 5 7 9] [ 234   8 ] [ 2345 789] [    5 789] 1           [ 2345  89]
4           [1 3   789] [  3 5 7 9] [ 23 567 9] [123  6 8 ] [123 5 789] [    56789] [ 23 56789] [ 23 56 89]
[1   5 789] 2           [  3 5 7 9] [  3 567 9] [1 34 6 8 ] [1 345 789] [    56789] [  3456789] [  3456 89]
...
If you want to play with different puzzles, the file [here](https://abhinavsarkar.net/files/sudoku17.txt.bz2?mtm_campaign=feed) lists some of the toughest ones. Let’s run[<sup>3</sup>](#fn3) some of them through our program to see how fast it is:
...
In this rather verbose article, we learned how to write a simple Sudoku solver in Haskell step-by-step. In the later parts of this series, we’ll delve into profiling the solution and figuring out better algorithms and data structures to solve Sudoku more efficiently. The code till now is available [here](https://code.abhinavsarkar.net/abhin4v/hasdoku/src/commit/0ef77341a10fcc25926301ee47b931d92959c0fa?mtm_campaign=feed).

 If you have any questions or comments, please leave a comment below. If you liked this post, please share it. Thanks for reading!

---

1. This exercise was originally done as a part of [the](https://github.com/pratul/haskell-classes/) [Haskell](https://github.com/ford-prefect/haskell-classes/) [classes](https://github.com/bnvinay92/haskell-classes/) I taught at [nilenso](https://nilenso.com).[↩︎](#fnref1)
...
2. `Alternative` implementation of `Maybe`:
   ...
   [↩︎](#fnref2)
3. All the runs were done on my MacBook Pro from 2014 with 2.2 GHz Intel Core i7 CPU and 16 GB memory.[↩︎](#fnref3)
...
* [Fediverse](https://fantastic.earth/@abnv/113901162100600307)
* [Lobsters](https://lobste.rs/s/trafke)
* [Reddit](https://www.reddit.com/r/haskell/comments/8uapij/)
* [My website](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-1/?mtm_campaign=feed#comment-container)
