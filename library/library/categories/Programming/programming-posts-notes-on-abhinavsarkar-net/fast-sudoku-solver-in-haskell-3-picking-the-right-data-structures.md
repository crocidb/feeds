...
description = "In the previous part in this series of posts, we optimized the simple Sudoku solver by implementing a new strategy to prune cells, and were able to achieve a speedup of almost 200x. Afterwards, we profiled the so"
date = "2018-08-13T00:00:00Z"
url = "https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-3/"
...
This post was originally published on [abhinavsarkar.net](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-3/?mtm_campaign=feed).
...
2. [A 200x Faster Solution](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-2/?mtm_campaign=feed)
3. **Picking the Right Data Structures** 👈

### Contents ###

1. [Quick Recap](#quick-recap)
2. [Profile Twice, Code Once](#profile-twice-code-once)
3. [A Set for All Occasions](#a-set-for-all-occasions)
4. [Bit by Bit, We Get Faster](#bit-by-bit-we-get-faster)
5. [Back to the Profiler](#back-to-the-profiler)
6. [Vectors of Speed](#vectors-of-speed)
7. [Revenge of the `(==)`](#revenge-of-the)
8. [One Function to Prune Them All](#one-function-to-prune-them-all)
9. [Rise of the Mutables](#rise-of-the-mutables)
10. [Comparison of Implementations](#comparison-of-implementations)
...
In the previous post, we improved the performance of the simple Sudoku solver by implementing a new strategy to prune cells. This [new strategy](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-2/?mtm_campaign=feed#a-little-forward-a-little-backward) found the digits which occurred uniquely, in pairs, or in triplets and fixed the cells to those digits. It led to a speedup of about 200x over our original naive solution. This is our current run[<sup>1</sup>](#fn1) time for solving all the 49151 [17-clue puzzles](https://abhinavsarkar.net/files/sudoku17.txt.bz2?mtm_campaign=feed):
...
Let’s try to improve this time.[<sup>2</sup>](#fn2)

Profile Twice, Code Once[#](#profile-twice-code-once)
----------
...
This generates a `sudoku.prof` file with the profiling output. Here are the top seven *Cost Centres*[<sup>3</sup>](#fn3) from the file (cleaned for brevity):
...
`pruneCell` uses `Data.List.\\` to find the difference of the cell’s possible digits and the fixed digits in the cell’s block. In Haskell, lists are implemented as [singly linked lists](https://en.wikipedia.org/wiki/Linked_list#Singly_linked_list). So, finding the difference or intersection of two lists is \\(O(n^2)\\), that is, quadratic [asymptotic complexity](https://en.wikipedia.org/wiki/Asymptotic_complexity). Let’s tackle this bottleneck first.

A Set for All Occasions[#](#a-set-for-all-occasions)
----------

What is a efficient data structure for finding differences and intersections? Why, a [*Set*](https://en.wikipedia.org/wiki/Set_(abstract_data_type)) of course! A Set stores unique values and provides fast operations for testing membership of its elements. If we use a Set to represent the possible values of cells instead of a List, the program should run faster. Since the possible values are already unique (`1`–`9`), it should not break anything.

Haskell comes with a bunch of Set implementations:

* [`Data.Set`](https://hackage.haskell.org/package/containers-0.6.0.1/docs/Data-Set.html) which is a generic data structure implemented as [self-balancing binary search tree](https://en.wikipedia.org/wiki/Self-balancing_binary_search_tree).
* [`Data.HashSet`](https://hackage.haskell.org/package/unordered-containers-0.2.9.0/docs/Data-HashSet.html) which is a generic data structure implemented as [hash array mapped trie](https://en.wikipedia.org/wiki/Hash_array_mapped_trie).
* [`Data.IntSet`](https://hackage.haskell.org/package/containers-0.6.0.1/docs/Data-IntSet.html) which is a specialized data structure for integer values, implemented as [radix tree](https://en.wikipedia.org/wiki/Radix_tree).

However, a much faster implementation is possible for our particular use-case. We can use a [*BitSet*](https://en.wikipedia.org/wiki/Bitset).

A BitSet uses [bits](https://en.wikipedia.org/wiki/Bit) to represent unique members of a Set. We map values to particular bits using some function. If the bit corresponding to a particular value is set to 1 then the value is present in the Set, else it is not. So, we need as many bits in a BitSet as the number of values in our domain, which makes is difficult to use for generic problems. But, for our Sudoku solver, we need to store only the digits `1`–`9` in the Set, which make BitSet very suitable for us. Also, the Set operations on BitSet are implemented using bit-level instructions in hardware, making them much faster than those on the other data structure listed above.

In Haskell, we can use the [`Data.Word`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Word.html) module to represent a BitSet. Specifically, we can use the [`Data.Word.Word16`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Word.html#t:Word16) type which has sixteen bits because we need only nine bits to represent the nine digits. The bit-level operations on `Word16` are provided by the [`Data.Bits`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Bits.html) module.

Bit by Bit, We Get Faster[#](#bit-by-bit-we-get-faster)
----------
...
We set the same bits as the digits to indicate the presence of the digits in the possibilities. For example, for digit `1`, we set the bit 1 so that the resulting `Word16` is `0000 0000 0000 0010` or 2. This also means, for fixed cells, the value is [count of the zeros from right](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Bits.html#v:countTrailingZeros).
...
```
> poss = Data.List.foldl' Data.Bits.setBit Data.Bits.zeroBits
> row = [Possible $ poss [4,6,9], Fixed $ poss [1], Fixed $ poss [5], Possible $ poss [6,9], Fixed $ poss [7], Possible $ poss [2,3,6,8,9], Possible $ poss [6,9], Possible $ poss [2,3,6,8,9], Possible $ poss [2,3,6,8,9]]
> putStr $ showGridWithPossibilities [row]
...
This is the same example row as the [last time](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-2/?mtm_campaign=feed#a-little-forward-a-little-backward). And it returns same results, excepts as a list of `Word16` now.
...
Wow! That is almost 3.7x faster than the previous solution. It’s a massive win! But let’s not be content yet. To the profiler again[<sup>4</sup>](#fn4)!

Back to the Profiler[#](#back-to-the-profiler)
----------
...
`exclusivePossibilities` is a big function. The profiler does not really tell us which parts of it are the slow ones. That’s because by default, the profiler only considers functions as *Cost Centres*. We need to give it hints for it to be able to find bottlenecks inside functions. For that, we need to insert [*Cost Centre* annotations](https://downloads.haskell.org/ghc/latest/docs/users_guide/profiling.html#inserting-cost-centres-by-hand) in the code:
...
Vectors of Speed[#](#vectors-of-speed)
----------

[Vector](https://hackage.haskell.org/package/vector-0.12.0.1) is a Haskell library for working with arrays. It implements very performant operations for integer-indexed array data. Unlike the lists in Haskell which are implemented as [singly linked lists](https://en.wikipedia.org/wiki/Linked_list#Singly_linked_list), vectors are stored in a contiguous set of memory locations. This makes random access to the elements a constant time operation. The memory overhead per additional item in vectors is also much smaller. Lists allocate memory for each item in the heap and have pointers to the memory locations in nodes, leading to a lot of wasted memory in holding pointers. On the other hand, operations on lists are lazy, whereas, operations on vectors are strict, and this may need to useless computation depending on the use-case[<sup>5</sup>](#fn5).
...
```
> Control.Monad.mapM_ print allRowIxs
[0,1,2,3,4,5,6,7,8]
...
> Control.Monad.mapM_ print allColIxs
...
> Control.Monad.mapM_ print allSubGridIxs
...
`readGrid` simply changes to work on a single vector of cells instead of a list of lists. Show functions have a pretty minor change to do lookups from a vector using the row indices and the [`(!)`](https://hackage.haskell.org/package/vector-0.12.0.1/docs/Data-Vector.html#v:-33-) function. The `(!)` function is the vector indexing function which is similar to the [`(!!)`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Prelude.html#v:-33--33-) function, except it executes in constant time.
...
All the three functions now take the grid and the cell indices instead of a list of cells, and use the cell indices to lookup the cells from the grid. Also, instead of using the [`traverse`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Data-Traversable.html#v:traverse) function as earlier, now we use the [`Control.Monad.foldM`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Control-Monad.html#v:foldM) function to fold over the cell-index-and-cell tuples in the context of the `Maybe` monad, making changes to the grid directly.
...
Here, the indentation indicated nesting of operations. We see that both the `(>>=)` and `basicUnsafeIndexM` functions—which together take around three-quarter of the run time—are being called from the `(==)` function in the `fixM` function[<sup>6</sup>](#fn6). It seems like we are checking for equality too many times. Here’s the usage of the `fixM` for reference:
...
One Function to Prune Them All[#](#one-function-to-prune-them-all)
----------

Why did we add `fixM` in the `pruneCells` function at all? Quoting from the [previous post](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-2/?mtm_campaign=feed#fn6),
...
We have merged the two pruning functions almost blindly. The important part here is the nested `pruneCell` function which uses monadic bind [`(>>=)`](https://hackage.haskell.org/package/base-4.11.1.0/docs/Control-Monad.html#v:-62--62--61-) to ensure that cells fixed in the first step are seen by the next step. Merging the two functions ensures that both strategies will see same *Exclusives* and *Fixeds*, thereby running in lockstep.
...
Ah, now it’s faster than the list-based implementation by 1.2x[<sup>7</sup>](#fn7). Let’s see what the profiler says:
...
Rise of the Mutables[#](#rise-of-the-mutables)
----------
...
What if, instead of using an immutable data structure for this, we used a mutable one? But how can we do that when we know that Haskell is a pure language? Purity means that all code must be [referentially transparent](https://en.wikipedia.org/wiki/Referential_transparency), and mutability certainly isn’t. It turns out, there is an escape hatch to mutability in Haskell. Quoting the relevant section from the book [Real World Haskell](https://web.archive.org/web/20180813/http://book.realworldhaskell.org/read/advanced-library-design-building-a-bloom-filter.html#id680273):
...
So if we use a mutable map in the [`ST` monad](https://hackage.haskell.org/package/base-4.11.1.0/docs/Control-Monad-ST.html), we may be able to get rid of this bottleneck. But, we can actually do better! Since the keys of our map are digits `1`–`9`, we can use a [mutable vector](https://hackage.haskell.org/package/vector-0.12.0.1/docs/Data-Vector-Mutable.html) to store the indices. In fact, we can go one step even further and store the indices as a BitSet as `Word16` because they also range from 1 to 9, and are unique for a block. This lets us use an [unboxed mutable vector](https://hackage.haskell.org/package/vector-0.12.0.1/docs/Data-Vector-Unboxed-Mutable.html). What is *unboxing* you ask? Quoting from the [GHC docs](https://downloads.haskell.org/ghc/latest/docs/users_guide/exts/primitives.html#unboxed-types):
...
When combined with vector, unboxing of values means the whole vector is stored as single byte array, avoiding pointer redirections completely. This is more memory efficient and allows better usage of caches[<sup>8</sup>](#fn8). Let’s rewrite `exclusivePossibilities` using `ST` and unboxed mutable vectors.
...
The run time is spread quite evenly over all the functions now and there are no hotspots anymore. We stop optimizating at this point[<sup>10</sup>](#fn10). Let’s see how far we have come up.
...
2. A lot of the code in this post references the code from the previous posts, including showing diffs. So, please read the previous posts if you have not already done so.[↩︎](#fnref2)

3. Notice the British English spelling of the word “Centre”. GHC was originally developed in [University of Glasgow](https://en.wikipedia.org/wiki/University_of_Glasgow) in Scotland.[↩︎](#fnref3)

4. The code for the BitSet based implementa­tion can be found [here](https://code.abhinavsarkar.net/abhin4v/hasdoku/src/commit/5a3044e09cd86dd6154bc50760095c4b38c48c6a?mtm_campaign=feed).[↩︎](#fnref4)

5. [This article](https://www.schoolofhaskell.com/user/commercial/content/vector) on School of Haskell goes into details about performance of vectors vs. lists. There are also [these](https://github.com/haskell-perf/sequences/blob/master/README.md) benchmarks for sequence data structures in Haskell: lists, vectors, seqs, etc.[↩︎](#fnref5)

6. We see Haskell’s laziness at work here. In the code for the `fixM` function, the `(==)` function is nested inside the `(>>=)` function, but because of laziness, they are actually evaluated in the reverse order. The evaluation of parameters for the `(==)` function causes the `(>>=)` function to be evaluated.[↩︎](#fnref6)

7. The code for the vector based implementa­tion can be found [here](https://code.abhinavsarkar.net/abhin4v/hasdoku/src/commit/a320a7874c6fa0c39665151cc8e073532cc750a1?mtm_campaign=feed).[↩︎](#fnref7)

8. Unboxed vectors have some [restrictions](https://hackage.haskell.org/package/vector-0.12.0.1/docs/Data-Vector-Unboxed.html#t:Unbox) on the kind of values that can be put into them but `Word16` already follows those restrictions so we are good.[↩︎](#fnref8)

9. Haskell can be a pretty good imperative programming language using the `ST` monad. [This article](https://vaibhavsagar.com/blog/2017/05/29/imperative-haskell/) shows how to implement some algorithms which require mutable data structures in Haskell.[↩︎](#fnref9)

10. The code for the mutable vector based implementation can be found [here](https://code.abhinavsarkar.net/abhin4v/hasdoku/src/commit/4a9a1531d5780e7abc7d5ab2a26dccbf34382031?mtm_campaign=feed).[↩︎](#fnref10)
...
* [Lobsters](https://lobste.rs/s/w3hlur)
* [Reddit](https://www.reddit.com/r/haskell/comments/96y0xa/)
* [My website](https://abhinavsarkar.net/posts/fast-sudoku-solver-in-haskell-3/?mtm_campaign=feed#comment-container)
