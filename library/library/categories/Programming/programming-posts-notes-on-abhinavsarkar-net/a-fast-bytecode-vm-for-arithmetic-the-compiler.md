+++
title = "A Fast Bytecode VM for Arithmetic: The Compiler"
...
url = "https://abhinavsarkar.net/posts/arithmetic-bytecode-vm-compiler/"
...
This post was originally published on [abhinavsarkar.net](https://abhinavsarkar.net/posts/arithmetic-bytecode-vm-compiler/?mtm_campaign=feed).
...
Bytecode is a flattened and compact representation of a program, usually manifested as a byte array. Bytecode is essentially an *[Instruction Set](https://en.wikipedia.org/wiki/Instruction_Set)* (IS), but custom-made to be executed by a *[Virtual Machine](https://en.wikipedia.org/wiki/Virtual_machine#Process_virtual_machines)* (VM), instead of a physical machine. Each bytecode instruction is one byte in size (that’s where it gets its name from). A bytecode and its VM are created in synergy so that the execution is as efficient as possible[<sup>1</sup>](#fn1). Compiling source code to bytecode and executing it in a VM also allows the program to be run on all platforms that the VM supports without the developer caring much about portability concerns. The most popular combo of bytecode and VM is probably the [Java bytecode](https://en.wikipedia.org/wiki/Java_bytecode) and the [Java virtual machine](https://en.wikipedia.org/wiki/Java_virtual_machine).

The VMs can be [stack-based](https://en.wikipedia.org/wiki/Stack_machine) or [register-based](https://en.wikipedia.org/wiki/Register_machine). In a stack-based VM, all values created during the execution of a program are stored only in a *[Stack](https://en.wikipedia.org/wiki/Stack_(abstract_data_type))* data-structure residing in the memory. Whereas, in a register-based VM, there is also an additional set of fixed number of [registers](https://en.wikipedia.org/wiki/Processor_register) that are used to store values in preference to the stack[<sup>2</sup>](#fn2). Register-based VMs are usually faster, but stack-based VMs are usually simpler to implement. For our purpose, we choose to implement a stack-based VM.
...
Let’s figure out the right bytecode for each case. First, we create *[Opcodes](https://en.wikipedia.org/wiki/Opcodes)* for each bytecode, which are sort of mnemonics for actual bytecode. Think of them as *[Assembly](https://en.wikipedia.org/wiki/Assembly_language)* is to *[Machine Code](https://en.wikipedia.org/wiki/Machine_Code)*.
...
Variables and `Let` expressions are more complex[<sup>3</sup>](#fn3). In the AST interpreter we chucked the variables in a map, but we cannot do that in a VM. There is no environment map in a VM, and all values must reside in the stack. How do we have variables at all then? Let’s think for a bit.
...
All the errors are thrown in the `IO` monad using the [`throwIO`](https://hackage.haskell.org/package/base/docs/Control-Exception.html#v:throwIO) function, and are caught after compilation using the [`catch`](https://hackage.haskell.org/package/base/docs/Control-Exception.html#v:catch) function. The final result or error is returned wrapped into [`Result`](https://abhinavsarkar.net/posts/arithmetic-bytecode-vm-parser/?mtm_campaign=feed#error-handling).
...
### Compiling, Fast and Slow[#](#compiling-fast-and-slow) ###
...
[![Compilation time using different data-structures](data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20800%20600'%3E%3C/svg%3E) \<img src="/images/plots/pandocplot5432109677417900652.svg" class="w-100pct" alt="Compilation time using different data-structures"\>\</img\>](https://abhinavsarkar.net/images/plots/pandocplot5432109677417900652.svg) Compilation time using different data-structures

The other important data structure used here is the map (or dictionary) in which we add the mappings from identifiers to their stack indices. This data structure needs to be performant because we do a lookup for each variable we encounter while compiling. I benchmarked compilation for some data structures[<sup>6</sup>](#fn6):

|                                                              Data structure                                                              |Time (ms)|Slowdown|
|------------------------------------------------------------------------------------------------------------------------------------------|---------|--------|
|    [`Data.HashMap.Strict.HashMap`](https://hackage.haskell.org/package/unordered-containers/docs/Data-HashMap-Strict.html#t:HashMap)     |   55    | 1.00x  |
|               [`Data.List.List`](https://hackage.haskell.org/package/base/docs/Data-List.html#t:List)[<sup>7</sup>](#fn7)                |   63    | 1.14x  |
|                 [`Data.Map.Strict.Map`](https://hackage.haskell.org/package/containers/docs/Data-Map-Strict.html#t:Map)                  |   71    | 1.29x  |
|                    [`Data.Trie.Trie`](https://hackage.haskell.org/package/bytestring-trie/docs/Data-Trie.html#t:Trie)                    |   80    | 1.45x  |
|[`Data.Vector.Hashtables.Dictionary`](https://hackage.haskell.org/package/vector-hashtables/docs/Data-Vector-Hashtables.html#t:Dictionary)|   104   | 1.89x  |
|    [`Data.HashTable.IO.BasicHashTable`](https://hackage.haskell.org/package/hashtables/docs/Data-HashTable-IO.html#t:BasicHashTable)     |   312   | 5.67x  |
...
[![Compilation time using different map data-structures](data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20800%20600'%3E%3C/svg%3E) \<img src="/images/plots/pandocplot15511258508283734502.svg" class="w-100pct" alt="Compilation time using different map data-structures"\>\</img\>](https://abhinavsarkar.net/images/plots/pandocplot15511258508283734502.svg) Compilation time using different map data-structures
...
Finally, I also tried converting the `go` function to be tail-recursive by using *[Continuation-passing style](https://en.wikipedia.org/wiki/Continuation-passing_style)* (CPS), and then [defunctionalizing](https://en.wikipedia.org/wiki/Defunctionalization) the continuations, as well as, using the [`ContT`](https://hackage.haskell.org/package/mtl/docs/Control-Monad-Cont.html#t:ContT) monad transformer. All of these approaches resulted in slower code. The times are interesting to compare (smaller is better):

[![Compilation time using different control-structures](data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20800%20600'%3E%3C/svg%3E) \<img src="/images/plots/pandocplot6077718961309221259.svg" class="w-100pct" alt="Compilation time using different control-structures"\>\</img\>](https://abhinavsarkar.net/images/plots/pandocplot6077718961309221259.svg) Compilation time using different control-structures
...
In the [next part](https://abhinavsarkar.net/posts/arithmetic-bytecode-vm/?mtm_campaign=feed), we write a virtual machine that runs our compiled bytecode, and do some benchmarking.

 If you have any questions or comments, please leave a comment below. If you liked this post, please share it. Thanks for reading!

---

1. There are VMs that execute hardware ISs instead of bytecode. Such VMs are also called *[Emulators](https://en.wikipedia.org/wiki/Emulators)* because they emulate actual CPU hardware. Some examples are [QEMU](https://www.qemu.org/) and [video game console emulators](https://en.wikipedia.org/wiki/Video_game_console_emulator).[↩︎](#fnref1)
...
6. I ran all benchmarks on an Apple M4 Pro 24GB machine against a 142MB file.[↩︎](#fnref6)

7. Used as *[Association List](https://en.wikipedia.org/wiki/Association_list)*.[↩︎](#fnref7)

8. The decompiler is a bottom-up [shift-reduce parser](https://en.wikipedia.org/wiki/shift-reduce_parser) from the opcodes to the expression tree.[↩︎](#fnref8)
...
* [Hacker News](https://news.ycombinator.com/item?id=45010406)
* [My website](https://abhinavsarkar.net/posts/arithmetic-bytecode-vm-compiler/?mtm_campaign=feed#comment-container)
