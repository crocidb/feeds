...
description = "I’ve been running a mixed Proxmox cluster for years – four nodes of wildly different capability, from an Atom x5-Z8350 with 2 GB of RAM (a z83ii, currently offline after years of faithful service as a basel"
date = "2026-06-18T18:45:00Z"
url = "https://taoofmac.com/space/blog/2026/06/18/1845?utm_content=atom"
author = "Rui Carmo"
text = ""
...
This year, somewhere along the way between writing [`agentbox`](https://github.com/rcarmo/agentbox?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link) and all the hype around agentic sandboxes I got tired of the eternal compromise between LXC containers and full virtual machines, and ended up building [`pve-microvm`](https://github.com/rcarmo/pve-microvm?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link) – a Debian package that adds QEMU’s `microvm` machine type as a first-class managed guest in [Proxmox](/space/os/linux/distributions/proxmox) VE.
...
It now ships a custom kernel, patches the [Perl](/space/dev/perl) internals to provide [Proxmox](/space/os/linux/distributions/proxmox) web UI integration, and, due to my usual fascination with offbeat operating systems, ended up supporting (as of this writing) 21 guest OS types from Debian to NetBSD to [Plan9](/space/os/plan9).

Yes, I completely brought it upon myself to run [Plan9](/space/os/plan9) in a microVM, and yes, it works.
...
](/space/blog/2026/06/18/1845#finding-the-right-balance)

After a few rounds of cluster cleanups and migrations, it’s now my daily driver for running [Gitea](/space/apps/gitea), Caddy reverse proxies, mini-firewalls, and the AI agent that’s helping me clean up this post.

[Proxmox](/space/os/linux/distributions/proxmox) gives you two main options out of the box:

* [LXC](/space/os/linux/lxc) containers start instantly, share the host kernel, and are spectacularly efficient. But they’re not *isolated* – a kernel exploit in one container compromises everything. You can’t run a different OS. You can’t easily nest Docker inside them without ending up (eventually) wrestling with `fuse-overlayfs` gymnastics. And certain workloads (anything needing custom kernel modules, or `CAP_SYS_ADMIN` in anger) simply don’t fit.
...
![Comparison of Standard VM, microVM, and LXC Container isolation and boot characteristics](https://taoofmac.com/media/blog/2026/06/18/1845/ycjiz2cThuEG3Atzjdf7MbCVX6w=/microvm-vs-lxc-vs-vm.svg)Comparison of Standard VM, microVM, and LXC Container isolation and boot characteristics

Now, let me be clear: I’m not spawning hundreds of these things. [I have Azure for that](https://github.com/rcarmo/azure-k3s-cluster?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link) – but I do want to run [Gitea](/space/apps/gitea) Actions workers, have a very limited set of hardware resources, and got fed up with the time it took for one particular VM to boot repeatedly…
...
](/space/blog/2026/06/18/1845#what-it-actually-does)
...
![How pve-microvm integrates with Proxmox VE internals](https://taoofmac.com/media/blog/2026/06/18/1845/Uu0L1y7Lqvf0bPpVCCmxqH2nURc=/microvm-architecture.svg)How pve-microvm integrates with Proxmox VE internals
...
](/space/blog/2026/06/18/1845#the-boot-sequence)
...
![Boot timeline comparison between microVM and standard VM](https://taoofmac.com/media/blog/2026/06/18/1845/-LaDyezTn_ik4nPTQr4EpujyaQc=/boot-sequence.svg)Boot timeline comparison between microVM and standard VM
...
I think the above is actually a bug in my kernel configuration, but haven’t really had time (or maybe even the right hardware) to tackle it – this is something I’d love more people to look at [and contribute patches](https://github.com/rcarmo/pve-microvm?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link).

[

### One Kernel, Many Guests ###

](/space/blog/2026/06/18/1845#one-kernel-many-guests)
...
](/space/blog/2026/06/18/1845#what-im-running)

On my cluster right now, I have a fair smattering of these already. Four off the top of my head are:

* `gitea` (VM 114, on an Intel N5105) – Bare-metal [Gitea](/space/apps/gitea) with SQLite, Caddy HTTPS, local actions runner, Avahi discovery. 2 cores, 2 GB RAM, 32 GB disk. Boots in \~3s, mostly because [Gitea](/space/apps/gitea) does a lot of housekeeping.
* `smith` (VM 9022, on my i7) – the main [`piclaw`](https://github.com/rcarmo/piclaw?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link) agent, the system that manages the cluster, releases [`piclaw`](https://github.com/rcarmo/piclaw?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link) and generally keeps tabs on everything. 2 cores, 6 GB RAM, 48 GB disk. Runs Docker internally, and has 3 smaller, volatile siblings scattered throughout the cluster that don’t run Docker but have different roles (CI/CD, wipe-and-reinstall agent instance for testing upgrades, etc.)
* `exo` (VM 9021, on my i7) – Distributed inference coordinator for running LLMs across multiple machines. CPU-only, 2 GB root.
* `virtualdsm` (VM 300, [`tnas`](/space/blog/2024/12/26/2330)) – Synology DSM running inside a microVM with Docker, inside Terramaster NAS hardware. Yeah, I know I’m weird, but it was [needed](/space/blog/2026/05/15/1330) when my Synology went sideways and I haven’t nuked it yet. Uses the stock Debian kernel rather than my custom one, because DSM needs specific module paths.

I also have a dormant 9Front ([Plan9](/space/os/plan9)) one, as well as a little menagerie of OpenWrt, OPNsense, OSv unikernels, gokrazy Go appliances, and various Alpine/Fedora/Rocky/Amazon Linux configurations filed away as standard [Proxmox](/space/os/linux/distributions/proxmox) backups. The 21 guest OS types aren’t theoretical – each one has been booted and validated, and sometimes `smith` will go and thaw one out to do regression tests.

The [`z83ii`](/space/blog/2017/12/03/2130) (that ancient Atom x5-Z8350 with 2GB RAM) was invaluable as a baseline test platform, because If a microVM can boot and run usefully on a fanless 2016-era Atom with 2 GB of total system memory, it’ll work anywhere. And it could run six before it started slowing down…
...
](/space/blog/2026/06/18/1845#what-a-config-looks-like)
...
](/space/blog/2026/06/18/1845#networking)

A microVM attaches to the network exactly like any other [Proxmox](/space/os/linux/distributions/proxmox) guest. The interface spec is a bit of a mouthful (`virtio-net-pci-non-transitional` device on the PCIe bus), and it lands on whatever Linux bridge and VLAN you point it at:
...
](/space/blog/2026/06/18/1845#storage-and-migration)

Storage was… trivial? every PVE backend works, because the disk is just a `virtio-blk-pci` device and [Proxmox](/space/os/linux/distributions/proxmox) hands it the same path it hands any VM. LVM and LVM-thin, ZFS, Ceph/RBD, NFS and CIFS, plain directory storage – all fine, with snapshots, linked and full clones, `vzdump` backups and `qm importdisk` behaving exactly as they do elsewhere. A `microvm` is a normal PVE guest that happens to boot differently.
...
](/space/blog/2026/06/18/1845#vs-lxc-when-to-choose-what)
...
](/space/blog/2026/06/18/1845#the-awkward-parts)

Now for the catches…

First one is pretty obvious: I’m patching someone else’s product, and even though I survived the [Perl](/space/dev/perl) 4 to Perl 5 transition decades ago *and* I have Codex to help these days, patching the Perl internals is fragile–every `qemu-server` upgrade can break my setup.
...
Another catch (for some) is that there is no VGA and no graphical console – the serial console is your only interface. If something goes wrong during boot, you’re reading kernel panics on a terminal. This is fine for servers, less fine for desktop-oriented guests, and [Plan9](/space/os/plan9) really doesn’t like it.

The next one is that *there’s no USB at all – no controllers, no passthrough, nothing on the bus* – so I had to get the web UI to hide those options the moment you flip a guest to `microvm`. This might be a deal-breaker if you want to, say, run a [Zigbee](/space/protocols/zigbee) controller (and is why my home automation stuff is still in an LXC).
...
](/space/blog/2026/06/18/1845#under-the-hood-the-patch-strategy)
...
](/space/blog/2026/06/18/1845#getting-started)

If you’ve read this far, you’re either a brave person or missed [the GitHub link](https://github.com/rcarmo/pve-microvm?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link), so I’ll just drop the potted version here:
...
](/space/blog/2026/06/18/1845#whats-next)
...
And, of course, someone should probably upstream this. I reached out informally to a couple of people at [Proxmox](/space/os/linux/distributions/proxmox) and was told I should join a mailing-list to discuss this, but a) that is awfully 90s and b) I just don’t have the time to do more than maintain this for myself. And yeah, I get that this would have to go through the entire enterprise QA pipeline (I [get that a lot, believe me](/space/blog/2025/10/15/0000)).

The source is at [github.com/rcarmo/pve-microvm](https://github.com/rcarmo/pve-microvm?utm_source=taoofmac.com&utm_medium=web&utm_campaign=unsolicited_traffic&utm_content=external_link), the patches are fairly small, it does not break [Proxmox](/space/os/linux/distributions/proxmox) itself since 99% of the features come from QEMU/KVM, so… I’m just literally giving it away.
