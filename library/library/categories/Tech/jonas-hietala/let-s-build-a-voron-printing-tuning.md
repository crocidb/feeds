...
description = "It’s up and running, and now it’s finally printing time! Less exciting—but necessary—tuning the printer to make the prints better.Filament shipmentEven though I’ve been building the printer for more than a month, I wasn’t ready for it being time for printing so soon. I "
date = "2024-07-17T19:35:07Z"
url = "http://jonashietala.se/blog/2023/11/30/lets_build_a_voron_printing_tuning/index.html"
author = "Jonas Hietala"
text = ""
lastupdated = "2025-10-22T08:58:11.289422759Z"
seen = true
---
...
* Polymaker PolyTerra PLA [Army Dark Green](https://www.3djake.com/polymaker/polyterra-pla-army-dark-green?sai=11933) and [Army Blue](https://www.3djake.com/polymaker/polyterra-pla-army-blue?sai=14897)
...
![](/images/trident/filament.jpg) I think I went overboard with the filament order. In hindsight I should’ve waited longer to see what materials I wanted to print, now I kind of regret buying so much PLA.

Secondary printer tuning
==========

At this point the next step in the VORON docs is [making a print](https://docs.vorondesign.com/build/slicer/first_print.html). But I had various issues, so I went through the [Secondary printer tuning](https://docs.vorondesign.com/tuning/secondary_printer_tuning.html) before getting my first successful print.
...
![](/images/trident/belt_frequencies.png) It’s around 110Hz, I think?
...
![](/images/trident/bed_mesh.png) The bed mesh is situated around 0, nothing unexpected here.

I also setup [Klipper Adaptive Meshing & Purging](https://github.com/kyleisah/Klipper-Adaptive-Meshing-Purging) (KAMP) using their setup instructions to dynamically adjust the bed to fit the print size.

Input shaping
----------

The guide references input shaping, which is included in the LDO kit. [NERO 3D](https://www.youtube.com/@Nero3D) recommended to wait with input shaping until you’ve printed with it a bit, so I did the input shaping after \~5 hours of printing time.

![](/images/trident/input_shaping.jpg) Input shaping connected.
...
The VORON docs [makes the first print seem so simple](https://docs.vorondesign.com/build/slicer/first_print.html)—just upload the print and eat a bowl of cereal. And I was looking forward to just hitting print and watching it go brr… But of course things wouldn’t go so smoothly.
...
1. Print stopped with `Hotend not enough`
   ...
   To fix it I had to manually set bed and hotend temperatures after Tap and modifying SuperSlicer to provide these values to the `PRINT_START` macro as detailed in [a better `PRINT_START` macro](https://github.com/jontek2/A-better-print_start-macro).
...
4. After a few failed attempts, the filament clogs.
   ...
   ![](/images/trident/clogged.jpg) The filament clogged somehow, and I had to disassemble the toolhead to fix it.
...
6. The print coming loose from the bed after a while.
   ...
   ![](/images/trident/fail_print.jpg) Oh FFS.
...
[![](/images/trident/first_cube_top.jpg)](/images/trident/first_cube_top.jpg) [![](/images/trident/first_cube_x.jpg)](/images/trident/first_cube_x.jpg)
...
High on adrenaline I set out to do my first functional print: an [exhaust cover](https://github.com/MotorDynamicsLab/LDOVoron2/blob/main/STLs/exhaust_cover.stl) for the back of the printer. I’m going to replace it with a proper filter in the future, but I wanted something to cover the big hole in the back when printing ABS, so I wanted to make a temporary in PLA.

But it things can’t go that smoothly:

![](/images/trident/fail_print_exhaust.jpg) I messed with the z-offset during print, and the result was this mess.
...
![](/images/trident/print_exhaust.jpg) Surprisingly good quality.

Other things weren’t that good:

![](/images/trident/not_great_nevermore_mount.jpg)
...
[Ellis’ print tuning guide](https://ellis3dp.com/Print-Tuning-Guide/) seems like the go-to guide for tuning your prints. It contains a *lot* of info, so I won’t write about it too much lest we’ll be here all day. These are the big things I did:
...
* Pressure advance
  ...
  I used the [pattern method](https://ellis3dp.com/Print-Tuning-Guide/articles/pressure_linear_advance/pattern_method.html), although there were *so* many settings I was unsure about so I don’t know how well it worked out.
...
![](/images/trident/printing_abs.jpg) Printer goes brr. ![](/images/trident/printed_parts.jpg) Some of the parts for a HEPA filter housing. If you zoom in you can see some stringing and very visible lines. ![](/images/trident/printed_parts2.jpg) The print lines are quite visible.
